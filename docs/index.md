...
Note: If your url matches a physical path, IIS will try to list directory contents (and you'll likely get a 403). The reason is that the `DirectoryListingModule` takes precedence over the `UrlRoutingModule`. There are a few options:

1. Set `RouteTable.Routes.RouteExistingFiles = true`. This will route all existing files and folder. This may have repercussions so you may have to ignore routes where physical files need to be served.
2. Reorder the `DirectoryListingModule` (IIS Manager > Server node > Modules then click View Ordered List and then move the `DirectoryListingModule` below the `UrlRoutingModule`).
3. Remove the `DirectoryListingModule` (IIS Manager > Server node > Modules select the `DirectoryListingModule` then click Delete).
3. Choose a different folder name that doesn't conflict.
</div>
